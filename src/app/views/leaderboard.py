...
class LeaderboardView(AppView):
    ...
    def on_show_view(self):
        ...
        title_label = arcade.gui.UILabel(
            text=Lang.get('leaderboard.title'), font_size=36
        )
        ...
        menu_button = arcade.gui.UIFlatButton(
            text=Lang.get('leaderboard.main_menu'), width=int(w * 0.25)
        )
        settings_button = arcade.gui.UIFlatButton(
            text=Lang.get('leaderboard.settings'), width=int(w * 0.25)
        )
        resume_button = arcade.gui.UIFlatButton(
            text=Lang.get('leaderboard.resume'), width=int(w * 0.25)
        )
    ...
    def _create_scores_box(self) -> arcade.gui.UIWidget:
        ...
        header_label = arcade.gui.UILabel(
            text=header_text, font_size=12, font_name='Arial'
        )
        ...
        if not top_scores:
            ...
            for idx, score_entry in enumerate(top_scores, 1):
                ...
                score_label = arcade.gui.UILabel(
                    text=score_text, font_size=12, font_name='Arial'
                )
