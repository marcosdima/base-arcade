...
def on_config_change(key: ConfigKey, value):
    if key in [ConfigKey.WINDOW_WIDTH, ConfigKey.WINDOW_HEIGHT]:
        window.set_size(
            value if key == ConfigKey.WINDOW_WIDTH else Config.get(ConfigKey.WINDOW_WIDTH, 800),
            value if key == ConfigKey.WINDOW_HEIGHT else Config.get(ConfigKey.WINDOW_HEIGHT, 600)
        )
