...
def test_point_parsing():
    ...
    assert p.x == 1 and p.y == 2, "Point parsing failed for tuple with more than 2 values."
...
def test_rect_contains():
    ...
    assert rect.contains(Point(5, 5)), "Rect contains method failed for a point inside the rect."
    assert rect.contains((5, 5)), "Rect contains method failed for a tuple point inside the rect."
    assert not rect.contains(Point(11, 5)), "Rect contains method failed for a point outside the rect."
    assert not rect.contains((11, 5)), "Rect contains method failed for a tuple point outside the rect."
